            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 hover:bg-white/15 ${
                    scrolled
            ...
            <div className="flex flex-col items-center justify-center h-full gap-6">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  onClick={() => setMobileOpen(false)}
                  className="text-2xl font-extrabold text-[#111] hover:text-[#0d4f4f] transition-colors"
                >
                  {link.label}
                </motion.a>
              ))}
